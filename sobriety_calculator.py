...
def format_output(total_days, delta_relative, lang="en"):
    ...
    склонение (частично, как мы обсуждали).
    """
    diff = delta_relative # для краткости
    
    # 1. Форматирование общей разницы в днях
    output_total_days = f"{MESSAGES['daysFrom'][lang]}: {total_days}"

    # 2. Форматирование относительной разницы (годы, месяцы, дни)
    parts = []
    ...
    # Всегда показываем дни, если нет лет/месяцев или если дни > 0
    ...
    if lang == "en":
        ...
        if len(parts) > 1:
            # Добавляем 'and' перед последним элементом
            last_item = parts[-1]
